...
for j in range(40, 80):
    ...
    #plt.plot(ts, np.array(ff_homo[j]) / 0.01 + j, color="red", alpha=0.5)
    ts = [time_step / 10.0 for time_step in range(len(ff_inhomo[j]))]    
    #plt.plot(ts, np.array(ff_inhomo[j]) / 0.01 + j, color="blue", alpha=0.5)    
    #plt.show()

    fpulse_homo   = np.array([ff_homo[j][time_step] if ts[time_step] < 190.0 + 2000.0 else 0.0 for time_step in range(len(ts))])
    fpulse_inhomo   = np.array([ff_inhomo[j][time_step] if ts[time_step] < 205.0 + 2000.0 else 0.0 for time_step in range(len(ts))])
